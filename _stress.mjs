...
function calcInverter(lp, panels, opts) {
  ...
  if (invSpec) {
    ...
    // Check: PV power compatibility
    if (totalPanelW > (invSpec.maxPVPower || invSpec.ratedW * 2)) {
      r.warnings.push('⚠️ قدرة الألواح ('+totalPanelW.toFixed(0)+'W) تتجاوز قدرة PV القصوى للعاكس ('+(invSpec.maxPVPower || invSpec.ratedW*2)+'W)');
      r.mpptCompatible = false;
    }
...
function calcParallelConfig(invSpec, numParallel, lp, totalPanelKW) {
  var r = {
  ...
  r.perInverterPanelCount = Math.round((totalPanelKW || 0) / numParallel * 1000 / invSpec.ratedW);
...
function handleCalculate() {
  ...
  var parallelConfig = calcParallelConfig(invSpec, parallelCount, lp, estimatedTotalKW);
